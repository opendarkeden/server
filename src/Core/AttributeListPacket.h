...
public:
    ...
    void write(SocketOutputStream& iStream) {
        ...
        m_Tail.write(oStream);
    }

    PacketSize_t getPacketSize() const {
        return sizeof(T) + m_Tail.getPacketSize();
    }
    string toString() const {
        return string(m_Value) + ", " + m_Tail.toString();
    }

    T getFirstValue() const {
        return m_Value;
    }
    AttributeListPacket<U>& getNextValues() {
        return m_Tail;
    }
    const AttributeListPacket<U>& getNextValues() const {
        return m_Tail;
    }
...
public:
    static void* getValue(AttributeListPacket<NullType>& packet) {
        return NULL;
    }
};
...
public:
    static T getValue(AttributeListPacket<Typelist<T, U>>& packet) {
        return packet.getFirstValue();
    }
};
...
public:
    static T getValue(AttributeListPacket<Typelist<T, U>>& packet) {
        return ValueAt<AttributeListPacket<U>, index - 1>::getValue(packet.getNextValues());
    }
};
