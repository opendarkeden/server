...
//			message << "전쟁이 시작되었습니다";
			...
			for (int i = 0; i < maxRelic; i++) {
				...
				try {
					...
					throw Error("can't find zone info");
                }
				...
				try {
					...
					throw Error("can't find zone group");
                }
	...
	throw(ProtocolException, Error)
{
	...
	if (pResult->next()) 
	{
        ...
        Statement* pStmt2 = g_pDatabaseManager->getConnection(Thread::self())->createStatement();
		...
		if (value1=="on")
		{
			...
			else
			{
				...
				if (!rTile.isAirBlocked() && !rTile.hasPortal())
				{
					...
					gcAddEffect.setDuration(Duration_t(999999));
