	 ...
	 throw(ProtocolException , Error)
{
	...
	if (DonationType == DONATION_TYPE_200501_PERSONAL || DonationType == DONATION_TYPE_200501_GUILD )
	{
		...
		// 기부 내용을 데이터 베이스에 기록한다.
		{
			...
			if (pPacket->getDonationType() == DONATION_TYPE_200501_PERSONAL )
			{
				...
				{
					pStmt = g_pDatabaseManager->getDistConnection("PLAYER_DB" )->createStatement();
					pStmt->executeQuery("INSERT INTO `DonationPersonal200501` VALUES ('%s', '%s', %d, %u, now())", pGamePlayer->getID().c_str(), pCreature->getName().c_str(), affectWorldID, pPacket->getGold());
				}
				END_DB(pStmt )
			}
			else if (pPacket->getDonationType() == DONATION_TYPE_200501_GUILD )
			{
				...
				{
					pStmt = g_pDatabaseManager->getDistConnection("PLAYER_DB" )->createStatement();
					pStmt->executeQuery("INSERT INTO `DonationGuild200501` VALUES (%u, '%s', '%s', '%s', %d, %u, now())", pPC->getGuildID(), pPC->getGuildName().c_str(), pGamePlayer->getID().c_str(), pCreature->getName().c_str(), affectWorldID, pPacket->getGold());
				}
				END_DB(pStmt )
			}
			else
	...
	else if (DonationType == DONATION_TYPE_200505_WEDDING )
	{
		...
		// DB 에 기록을 남긴다.
		{
			...
			{
				pStmt = g_pDatabaseManager->getDistConnection("PLAYER_DB")->createStatement();
				pStmt->executeQuery("INSERT INTO `DonationWedding200505` VALUES ('%s', '%s', %d, %u, now())", pGamePlayer->getID().c_str(), pCreature->getName().c_str(), affectWorldID, pPacket->getGold());
			}
			END_DB(pStmt )
		}
