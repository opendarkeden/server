...
private:
    ...
    enum TimeLimitStatus { VALID = 0, EXPIRED, SOLD, MORPH, EXTENDED };

public:
    typedef unordered_map<ObjectID_t, VSDateTime> ItemTimeLimitMap;

public:
    TimeLimitItemManager(PlayerCreature* pOwner) {
        m_pOwnerPC = pOwner;
...
public:
    ...
    bool itemSold(Item* pItem) {
        return changeStatus(pItem, SOLD);
    }
    bool deleteItemByMorph(Item* pItem) {
        return changeStatus(pItem, MORPH);
    }
