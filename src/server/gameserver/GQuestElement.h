...
class GQuestElement {
public:
    enum ResultType { FAIL = 0, OK, WAIT };

    GQuestElement() : m_pParent(NULL), m_Condition(0), m_Index(0) {}
    virtual ~GQuestElement() {}

    virtual string getElementName() const = 0;
    virtual ResultType checkCondition(PlayerCreature* pPC) const {
        return WAIT;
    }
    virtual ResultType checkMission(GQuestMission* pMission) const {
        return FAIL;
    }

    virtual GQuestMission* makeInitMission(PlayerCreature* pPC) const {
        return NULL;
    }

    virtual GQuestManager::EventTypes getEventType() const {
        return GQuestManager::MAX;
    }
    ...
    void setParent(GQuestInfo* pParent) {
        m_pParent = pParent;
    }
