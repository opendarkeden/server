...
public:
    ...
    list<GQuestMission*>& getEventMission(EventTypes type) {
        return m_EventMissions[type];
    }
    const list<GQuestMission*>& getEventMission(EventTypes type) const {
        return m_EventMissions[type];
    }

    GQuestInventory& getGQuestInventory() {
        return m_GQuestInventory;
    }
    const GQuestInventory& getGQuestInventory() const {
        return m_GQuestInventory;
    }
    ...
    bool canEnterDynamicZone(ZoneID_t zoneID) {
        return m_DynamicZoneInfo[zoneID] != 0;
    }
    void setEnterDynamicZone(ZoneID_t zoneID, BYTE enter) {
        m_DynamicZoneInfo[zoneID] = enter;
    }
