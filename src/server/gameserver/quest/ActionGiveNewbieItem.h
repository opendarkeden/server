...
public:
    virtual ActionType_t getActionType() const {
        return ACTION_GIVE_NEWBIE_ITEM;
    }
...
public:
    uint getItemClass(void) const {
        return m_ItemClass;
    }
    void setItemClass(uint IClass) {
        m_ItemClass = IClass;
    }
...
public:
    virtual ActionType_t getActionType() const {
        return Action::ACTION_GIVE_NEWBIE_ITEM;
    }
    virtual string getActionName() const {
        return "GiveNewbieItem";
    }
    virtual Action* createAction() const {
        return new ActionGiveNewbieItem();
    }
};
