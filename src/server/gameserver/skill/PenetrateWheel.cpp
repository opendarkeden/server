	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bTileCheck && bSatisfyRequire)
		{
			...
			for ( oY = Y - 2 ; oY <= Y + 2 ; ++oY )
			{
				...
				// 이펙트 오브젝트를 생성한다.
				EffectPenetrateWheel* pEffect = new EffectPenetrateWheel(pZone, oX, oY);
				...
				/*if ( Grade > 0 )
				{
					if ( Grade == 1 ) pEffect->setSendEffectClass( Effect::EFFECT_CLASS_Penetrate_Wheel_2 );
					else pEffect->setSendEffectClass( Effect::EFFECT_CLASS_Penetrate_Wheel_3 );
				}*/
