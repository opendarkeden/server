...
template <class T> class NameManager {
public:
...
template <class T> bool NameManager<T>::add(const string& name, const T& data) {
...
template <class T> bool NameManager<T>::remove(const string& name) {
...
template <class T> bool NameManager<T>::isExist(const string& name) const {
    typename NameHashMap::const_iterator itr = m_Names.find(name);
