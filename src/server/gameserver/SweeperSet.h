...
class SweeperSet {
public:
    enum SweeperIndex { SWEEPER_SLAYER, SWEEPER_VAMPIRE, SWEEPER_OUSTERS, SWEEPER_DEFAULT };

    SweeperSet(SweeperIndex idx) : m_Index(idx) {
        //	m_SweeperSafes.reserve(12);
    }

    void addSafe(uint itemType, MonsterCorpse* pSafe) {
        m_SweeperSafes[itemType] = pSafe;
    }
    ...
    //	bool	isSafe( MonsterCorpse* pCorpse ) const { return find( m_SweeperSafes.begin(), m_SweeperSafes.end(),
    // pCorpse ) != m_SweeperSafes.end(); }
...
public:
    ...
    SweeperSet* getSweeperSet(uint index) {
        return m_SweeperSets[index];
    }
