...
GQuestElement::ResultType GQuestAdvancementClassLevelElement::checkCondition(PlayerCreature* pPC) const {
    if (pPC->getAdvancementClassLevel() <= m_Max && pPC->getAdvancementClassLevel() >= m_Min)
        return OK;
    else if (m_Index == 0)
        return FAIL;

    if (pPC->getAdvancementClassLevel() < m_Min)
        return WAIT;
    return FAIL;
}
...
GQuestElement::ResultType GQuestAdvancementClassLevelElement::checkMission(GQuestMission* pStatus) const {
    GQuestAdvancementClassLevelMission* pAdvancementClassLevelMission =
        dynamic_cast<GQuestAdvancementClassLevelMission*>(pStatus);
    if (pAdvancementClassLevelMission == NULL)
        return FAIL;

    if (pAdvancementClassLevelMission->isSuccess())
        return OK;
    return WAIT;
}
