	...
	BEGIN_DB
	{
		...
		while (pResult->next()) {
			...
			if (sta != QuestStatusInfo::COMPLETE && sta != QuestStatusInfo::FAIL && sta != QuestStatusInfo::CAN_REPLAY )
				filelog("GQuestError.log", "Invalid Status stored in Quest : [%s]:%d/%d", m_pOwner->getName().c_str(), qID, sta);
			else {
...
void GQuestManager::refreshQuest(bool sendPacket)
{
	...
	map<DWORD, GQuestInfo*>& infos = GQuestInfoManager::Instance().getInfos();

	map<DWORD, GQuestInfo*>::iterator itr = infos.begin();
	map<DWORD, GQuestInfo*>::iterator endItr = infos.end();

	for (; itr != endItr ; ++itr ) {
//		if (m_QuestStatuses[itr->first] != NULL ) continue;
		map<DWORD, GQuestStatus*>::iterator finditr = m_QuestStatuses.find(itr->first);
...
Packet* GQuestManager::getStatusInfoPacket() const
{
	...
	map<DWORD, GQuestStatus*>::const_iterator itr = m_QuestStatuses.begin();
	map<DWORD, GQuestStatus*>::const_iterator endItr = m_QuestStatuses.end();
...
{
	map<DWORD, GQuestStatus*>::iterator itr = m_QuestStatuses.find(qID);
...
void GQuestManager::blooddrain()
{
	...
	while (itr != m_EventMissions[BLOODDRAIN].end() )
	{
		...
		{
			sprintf(buffer, "Drained blood %u times.", pBloodDrainMission->getCurrent());
		...
		{
			sprintf(buffer, "Absorbed souls %u times.", pBloodDrainMission->getCurrent());
			GCSystemMessage gcSM;
			gcSM.setMessage(buffer);
			m_pOwner->getPlayer()->sendPacket(&gcSM);
		}
...
bool GQuestManager::metNPC(NPC* pNPC)
{
	...
	while (itr != m_EventMissions[MEETNPC].end() )
	{
		...
		{
			//cout << "SayNPCElement 某胶泼 角菩!!!!" << endl;
			Assert(false);
		}
...
void GQuestManager::eraseQuest(DWORD qID) {
	...
	BEGIN_DB {
		pStmt = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
		pStmt->executeQuery("DELETE FROM GQuestSave WHERE OwnerID='%s' AND QuestID='%u'", m_pOwner->getName().c_str(), qID);
