...
public:
    ...
    bool isTarget(DWORD target) {
        return find(m_TargetList.begin(), m_TargetList.end(), target) != m_TargetList.end();
    }
    bool isVisited(DWORD target) {
        return find(m_VisitedList.begin(), m_VisitedList.end(), target) != m_VisitedList.end();
    }
    ...
    vector<DWORD>& getTargetList() {
        return m_TargetList;
    }
    vector<DWORD>& getVisitedList() {
...
public:
    ...
    DWORD getGrade() const {
        return m_Grade;
    }
    DWORD getBaseNum() const {
        return m_BaseNum;
    }
