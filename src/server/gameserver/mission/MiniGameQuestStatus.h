...
public:
    MiniGameQuestStatus(QuestID_t qID, const VSDateTime& deadline, int gameType)
        : QuestStatus(qID, deadline, QUEST_CLASS_MINI_GAME), m_GameType(gameType) {
        m_bScoreSet = false;
        m_GameScore = 0;
    }
    ...
    bool setScore(uint score) {
        if (m_bScoreSet)
            return false;
        m_GameScore = score;
        return m_bScoreSet = true;
    }
