...
void GDRLairManager::run() throw()
{
	...
	cout << "[GAMESERVER] GDRLairManager connected to the DB." << endl;
...
/*	if (time.hour()%2 )
	...
	filelog("GDRLair.log", "[GDRLair] Restart at %s", ret.toString().c_str());
...
void GDRLairEntrance::start()
{
	...
	gcSM.setMessage("Gilles De Rais Lair is now open.");
...
void GDRLairIcepole::start()
{
	...
	// 일루젼스웨이에서는 몽땅 쫓아내버린다.
	GCSystemMessage gcSM;
	gcSM.setMessage("Failed to pass the Illusion Way, resurrection in 10 seconds.");
...
*/
	...
	const PCManager* pPM = pZone->getPCManager();
	const map<ObjectID_t, Creature*>& pCL = pPM->getCreatures();

	map<ObjectID_t, Creature*>::const_iterator itr = pCL.begin();
	map<ObjectID_t, Creature*>::const_iterator endItr = pCL.end();
...
void GDRLairScene6::start()
{
	...
	for (itr=creatures.begin(); itr!=creatures.end(); itr++)
	{
		...
		if (pCreature->isPC()) {
			...
			filelog("GDRLair.log", "%s has received a bead.", pPC->getName().c_str());
//				itemType = ((goodOneIndex[1]==i||goodOneIndex[2]==i)? 9:8);
			...
			{
				filelog("GDRLair.log", "No place in inventory.");
				SAFE_DELETE(pItem);
			}
		}
	}
	...
	for (i=0, itr=creatures.begin(); itr!=creatures.end(); i++, itr++) {
		...
		if (pCreature->isPC()) {
			...
			if (rewardType[i] == 0) {
				...
				filelog("GDRLair.log", "%s has received a CoreZap. (%d/%d)", pPC->getName().c_str(), itemType, grade);
			} else {
				if (rewardType[i] == 1) {
					itemType = 9;
					filelog("GDRLair.log", "%s has received the pendant.", pPC->getName().c_str());
					list<OptionType_t> nullList;
					pItem = g_pItemFactoryManager->createItem(Item::ITEM_CLASS_QUEST_ITEM, itemType, nullList);
				} else {
//					itemType = 8;
...
void GDRLairKillAll::start()
{
	...
	GCSystemMessage gcSM;
	gcSM.setMessage("Failed to find GDR lair, resurrection in 10 seconds.");
