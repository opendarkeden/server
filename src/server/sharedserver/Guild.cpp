...
GuildMember::GuildMember() throw() {
	m_bLogOn = false;
//	m_ServerID = 255;
}

void GuildMember::create() throw() {
	__BEGIN_TRY
	...
	BEGIN_DB {
		...
		if (pResult->getRowCount() != 0) {
			...
			if (m_Rank == GUILDMEMBER_RANK_WAIT )
				pStmt->executeQuery("UPDATE GuildMember SET GuildID = %d, Rank = %d, ExpireDate = '', RequestDateTime = '%s' WHERE Name = '%s'", m_GuildID, m_Rank, getRequestDateTime().c_str(), m_Name.c_str());
			else
				pStmt->executeQuery("UPDATE GuildMember SET GuildID = %d, Rank = %d, ExpireDate = '' WHERE Name = '%s'", m_GuildID, m_Rank, m_Name.c_str());
		} else {
			if (m_Rank == GUILDMEMBER_RANK_WAIT)
				pStmt->executeQuery("INSERT INTO GuildMember(GuildID, Name, Rank, RequestDateTime ) VALUES (%d, '%s', %d, '%s' )", m_GuildID, m_Name.c_str(),  m_Rank, getRequestDateTime().c_str());
			else
				pStmt->executeQuery("INSERT INTO GuildMember(GuildID, Name, Rank ) VALUES (%d, '%s', %d )", m_GuildID, m_Name.c_str(),  m_Rank);
		}
...
void GuildMember::save() throw() {
	...
	BEGIN_DB {
		...
		pStmt->executeQuery("UPDATE GuildMember SET GuildID = %d, Rank = %d WHERE Name = '%s'",	m_GuildID, m_Rank, m_Name.c_str());
...
void GuildMember::destroy() throw() {
	__BEGIN_TRY
...
void GuildMember::saveIntro(const string& intro) throw() {
	__BEGIN_TRY
...
string GuildMember::getIntro() const throw() {
	__BEGIN_TRY
...
string GuildMember::toString() const throw() {
	__BEGIN_TRY
...
string GuildMember::getRequestDateTime() const throw() {
	__BEGIN_TRY

	char buf[20];

	sprintf(buf, "%4d-%02d-%02d %02d:%02d:%02d", m_RequestDateTime.date().year(), m_RequestDateTime.date().month(), m_RequestDateTime.date().day(),m_RequestDateTime.time().hour(), m_RequestDateTime.time().minute(), m_RequestDateTime.time().second());
...
void GuildMember::setRank(GuildMemberRank_t rank) throw() {
	__BEGIN_TRY
...
void GuildMember::setRequestDateTime(const string& rtime) throw() {
	__BEGIN_TRY
...
bool GuildMember::isRequestDateTimeOut(const VSDateTime& currentDateTime ) const throw() {
	__BEGIN_TRY
...
void Guild::create() throw() {
	...
	BEGIN_DB {
		...
		pStmt->executeQuery("INSERT INTO GuildInfo (GuildID, GuildName, GuildType, GuildRace, GuildState, ServerGroupID, GuildZoneID, Master, Date, Intro ) VALUES (%d, '%s', %d, %d, %d, %d, %d, '%s', '%s', '%s' )", m_ID, m_Name.c_str(), m_Type, m_Race, m_State, m_ServerGroupID, m_ZoneID, m_Master.c_str(), m_Date.c_str(), correctIntro.c_str());
...
void Guild::save() throw() {
	...
	BEGIN_DB {
		...
		pStmt->executeQuery("UPDATE GuildInfo SET GuildName = '%s', GuildType = %d, GuildRace = %d, GuildState = %d, ServerGroupID = %d, GuildZoneID = %d, Master = '%s', Date = '%s' WHERE GuildID = %d", m_Name.c_str(), m_Type, m_Race, m_State, m_ServerGroupID, m_ZoneID, m_Master.c_str(), m_Date.c_str(), m_ID);
...
#ifdef __SHARED_SERVER__
void Guild::saveIntro(const string& intro) throw() {
	__BEGIN_TRY
...
void Guild::tinysave(const char* field) const throw() {
	__BEGIN_TRY
...
GuildMember* Guild::getMember(const string& name) const throw() {
	__BEGIN_TRY
...
GuildMember* Guild::getMember_NOLOCKED(const string& name) const throw() {
	__BEGIN_TRY
...
void Guild::addMember(GuildMember* pMember) throw(DuplicatedException, Error) {
	__BEGIN_TRY
	__ENTER_CRITICAL_SECTION(m_Mutex)
...
void Guild::deleteMember(const string& name) throw() {
	__BEGIN_TRY
	__ENTER_CRITICAL_SECTION(m_Mutex)
...
void Guild::modifyMember(GuildMember& Member) throw() {
	__BEGIN_TRY
	__ENTER_CRITICAL_SECTION(m_Mutex)
...
void Guild::modifyMemberRank(const string& name, GuildMemberRank_t rank) throw() {
	__BEGIN_TRY
...
#ifdef __GAME_SERVER__
void Guild::addCurrentMember(const string& name) throw(DuplicatedException, Error) {
	__BEGIN_TRY
...
void Guild::deleteCurrentMember(const string& name) throw(NoSuchElementException) {
	__BEGIN_TRY
	__ENTER_CRITICAL_SECTION(m_Mutex)
...
list<string> Guild::getCurrentMembers() throw() {
	__BEGIN_TRY
...
#ifdef __SHARED_SERVER__
void Guild::makeInfo(GuildInfo2* pGuildInfo) throw() {
	__BEGIN_TRY
	__ENTER_CRITICAL_SECTION(m_Mutex)
...
void Guild::makeInfo(GuildInfo* pGuildInfo) throw() {
	__BEGIN_TRY
	__ENTER_CRITICAL_SECTION(m_Mutex)
...
void Guild::makeMemberInfo(GCGuildMemberList& gcGuildMemberList) throw() {
	__BEGIN_TRY
	__ENTER_CRITICAL_SECTION(m_Mutex)
...
void Guild::expireTimeOutWaitMember(VSDateTime currentDateTime, list<string>& mList) throw(Error) {
	__BEGIN_TRY
...
string Guild::toString() const throw() {
	__BEGIN_TRY
...
string Guild::correctString(const string& str) throw() {
	__BEGIN_TRY
