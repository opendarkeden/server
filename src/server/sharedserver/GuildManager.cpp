...
GuildManager::GuildManager() throw() {
    __BEGIN_TRY
...
GuildManager::~GuildManager() throw() {
    ...
    map<GuildID_t, Guild*>::iterator itr = m_Guilds.begin();
...
void GuildManager::load() throw() {
    ...
    BEGIN_DB {
        // 길드 정보를 DB로 부터 읽어온다.
        pStmt = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
        pResult = pStmt->executeQuery("SELECT GuildID, GuildName, GuildType, GuildRace, GuildState, ServerGroupID, GuildZoneID, Master, Date, Intro FROM GuildInfo WHERE GuildState IN (%d, %d)", Guild::GUILD_STATE_WAIT, Guild::GUILD_STATE_ACTIVE);
...
void GuildManager::addGuild(Guild* pGuild) throw(DuplicatedException) {
    __BEGIN_TRY
    ...
    map<GuildID_t, Guild*>::iterator itr = m_Guilds.find(pGuild->getID());
    if (itr != m_Guilds.end())
...
void GuildManager::addGuild_NOBLOCKED(Guild* pGuild) throw(DuplicatedException) {
    __BEGIN_TRY
...
void GuildManager::deleteGuild(GuildID_t id) throw(NoSuchElementException) {
    __BEGIN_TRY
    __ENTER_CRITICAL_SECTION(m_Mutex)

    map<GuildID_t, Guild*>::iterator itr = m_Guilds.find(id);
    if (itr == m_Guilds.end())
...

    {
        ...
        map<ZoneID_t, CastleInfo*>::const_iterator endItr = castleInfos.end();
...
Guild* GuildManager::getGuild(GuildID_t id) throw() {
    __BEGIN_TRY
...
Guild* GuildManager::getGuild_NOBLOCKED(GuildID_t id) throw() {
    __BEGIN_TRY
...
#ifdef __SHARED_SERVER__
void GuildManager::makeSGGuildInfo(SGGuildInfo& sgGuildInfo) throw() {
    __BEGIN_TRY
    __ENTER_CRITICAL_SECTION(m_Mutex)
...
void GuildManager::makeWaitGuildList(GCWaitGuildList& gcWaitGuildList, GuildRace_t race) throw() {
    __BEGIN_TRY
    __ENTER_CRITICAL_SECTION(m_Mutex)
...
void GuildManager::makeActiveGuildList(GCActiveGuildList& gcActiveGuildList, GuildRace_t race) throw() {
    __BEGIN_TRY
    __ENTER_CRITICAL_SECTION(m_Mutex)
...
void GuildManager::heartbeat() throw(Error) {
    __BEGIN_TRY
...
string GuildManager::toString() const throw() {
    __BEGIN_TRY
...
bool GuildManager::isGuildMaster(GuildID_t guildID, PlayerCreature* pPC) throw(Error) {
#ifdef __GAME_SERVER__
    __BEGIN_TRY
...
// 길드가 성을 가졌나?
bool GuildManager::hasCastle(GuildID_t guildID) throw(Error) {
    __BEGIN_TRY
...
// 길드가 성을 가졌나?
bool GuildManager::hasCastle(GuildID_t guildID, ServerID_t& serverID, ZoneID_t& zoneID) throw(Error) {
    __BEGIN_TRY
    ...
    BEGIN_DB {
        pStmt = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
        Result* pResult = pStmt->executeQuery("SELECT ServerID, ZoneID FROM CastleInfo WHERE GuildID = %d", (int)guildID);
...
// 길드가 전쟁신청을 했나?
bool GuildManager::hasWarSchedule(GuildID_t guildID) throw(Error) {
    __BEGIN_TRY
    ...
    BEGIN_DB {
        ...
        Result* pResult = pStmt->executeQuery("SELECT count(*) FROM WarScheduleInfo WHERE (AttackGuildID = %d OR AttackGuildID2 = %d OR AttackGuildID3 = %d OR AttackGuildID4 = %d OR AttackGuildID5 = %d) AND Status in ('WAIT', 'START')", (int)guildID, (int)guildID, (int)guildID, (int)guildID, (int)guildID);
#else
        Result* pResult = pStmt->executeQuery("SELECT count(*) FROM WarScheduleInfo WHERE AttackGuildID = %d AND Status<>'END'", (int)guildID);
#endif
...
bool GuildManager::hasActiveWar(GuildID_t guildID) throw(Error) {
    __BEGIN_TRY
    ...
    if (hasCastle(guildID, serverID, zoneID)) {
        ...
        BEGIN_DB {
            pStmt = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
            Result* pResult = pStmt->executeQuery("SELECT count(*) FROM WarScheduleInfo WHERE ServerID = %u AND ZoneID = %u AND Status = 'START'", serverID, zoneID);
        ...
        BEGIN_DB {
            pStmt = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
            Result* pResult = pStmt->executeQuery("SELECT count(*) FROM WarScheduleInfo WHERE AttackGuildID = %d AND Status = 'START'", (int)guildID);
...
string GuildManager::getGuildName(GuildID_t guildID) throw(Error) {
    __BEGIN_TRY
