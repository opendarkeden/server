...
void ResurrectLocationManager::load() throw() {
    ...
    BEGIN_DB {
        ...
        if (pResult->getRowCount() == 0) {
            cerr << "[ResurrectLocationManager] TABLE DOES NOT EXIST." << endl;
            throw("[ResurrectLocationManager] TABLE DOES NOT EXIST.");
        }
...
bool ResurrectLocationManager::getSlayerPosition(ZoneID_t id, ZONE_COORD& zoneCoord) const throw() {//NoSuchElementException)
    __BEGIN_TRY

    map<ZoneID_t, ZONE_COORD>::const_iterator itr = m_SlayerPosition.find(id);

    if (itr == m_SlayerPosition.end()) {
        cerr << "[ResurrectLocationManager] No Such ZoneID." << endl;
...
void ResurrectLocationManager::addSlayerPosition(ZoneID_t id, const ZONE_COORD& coord) throw(DuplicatedException, Error) {
    __BEGIN_TRY

    map<ZoneID_t, ZONE_COORD>::const_iterator itr = m_SlayerPosition.find(id);

    if (itr != m_SlayerPosition.end()) {
        cerr << "[ResurrectLocationManager] ZoneID already exist!" << endl;
        throw NoSuchElementException("[ResurrectLocationManager] ZoneID already exist!");
    }
...
bool ResurrectLocationManager::getVampirePosition(ZoneID_t id, ZONE_COORD& zoneCoord) const throw() {//NoSuchElementException)
    __BEGIN_TRY
...
void ResurrectLocationManager::addVampirePosition(ZoneID_t id, const ZONE_COORD& coord) throw(DuplicatedException, Error) {
    __BEGIN_TRY
