...
void PaySystem::setPayPlayAvailableDateTime(const string& pat) throw(Error) {
    __BEGIN_TRY
...
void PaySystem::setFamilyPayPlayAvailableDateTime(const string& pat) throw(Error) {
    __BEGIN_TRY
...
void PaySystem::setPayStartAvailableDateTime(const string& pat) throw(Error) {
    __BEGIN_TRY
...
bool PaySystem::checkPayPlayAvailable() {
    if (m_PayType!=PAY_TYPE_FREE && m_PayType!=PAY_TYPE_POST) {
        ...
        // 패밀리 요금제 사용가능한지 체크한다.
        if (m_PayPlayAvailableDateTime != m_FamilyPayPlayAvailableDateTime && currentDateTime <= m_FamilyPayPlayAvailableDateTime) {
...
void PaySystem::setPayPlayValue(PayType payType, const string& payPlayDate, int payPlayHours, uint payPlayFlag, const string& familyPayPlayDate) {
    setPayType(payType);
...
bool PaySystem::updatePayPlayTime(const string& playerID, const VSDateTime& currentDateTime, const Timeval& currentTime) throw(ProtocolException, Error) {
    __BEGIN_TRY
...
// 그 정보를 읽어들인다.
//---------------------------------------------------------------------------
bool PaySystem::loginPayPlayPCRoom(const string& ip, const string& playerID) throw(Error) {
    __BEGIN_TRY
    ...
    BEGIN_DB {
        ...
        try {
            pResult = pStmt->executeQuery(
                "SELECT r.ID, r.PayType, r.PayStartDate, r.PayPlayDate, r.PayPlayHours, r.PayPlayFlag, r.UserLimit, r.UserMax FROM PCRoomInfo r, PCRoomIPInfo p WHERE p.IP='%s' AND p.ID=r.ID", ip.c_str());
        } catch (Throwable& t) {
            filelog("paySystem.txt", "%s", t.toString().c_str());
            throw;
        }
        ...
        if (pResult!=NULL && pResult->next()) {
            ...
            if (pResult->next()) {
                ...
                //
                if (m_PayType==PAY_TYPE_PERIOD && users >= m_UserLimit || m_PayType==PAY_TYPE_TIME && users >= m_UserMax) {
                ...
                if (bAvailable) {
                    // 일단 사용자를 추가해보는데..
                    try {
                        pStmt->executeQuery("INSERT IGNORE INTO `PCRoomUserInfo`(`ID`, `PlayerID`) VALUES(%d, '%s')", m_PCRoomID, playerID.c_str());
...
void PaySystem::logoutPayPlayPCRoom(const string& playerID) throw(Error) {
    __BEGIN_TRY
...
bool PaySystem::loginPayPlay(PayType payType, const string& payPlayDate, int payPlayHours, uint payPlayFlag, const string& ip, const string& playerID) throw(Error) {
    __BEGIN_TRY
...
bool PaySystem::loginPayPlay(const string& ip, const string& playerID) throw(Error) {
...
void PaySystem::logoutPayPlay(const string& playerID, bool bClear, bool bDecreaseTime) throw(Error) {
    __BEGIN_TRY
...
void PaySystem::clearPayPlayDateTime(const string& playerID) throw(Error) {
    __BEGIN_TRY
    ...
    BEGIN_DB {
        ...
        pStmt->executeQuery("UPDATE `Player` SET `PayPlayHours` = 0, `PayPlayDate` = '2002-11-18 00:00:00' WHERE `PlayerID`='%s'", playerID.c_str());
...
void PaySystem::decreasePayPlayTime(const string& playerID, uint mm) throw(Error) {
    __BEGIN_TRY
...
void PaySystem::decreasePayPlayTimePCRoom(uint mm) throw(Error) {
    __BEGIN_TRY
...
void PaySystem::increasePayPlayTimePCRoom(uint mm) throw(Error) {
    __BEGIN_TRY
    ...
    BEGIN_DB {
        ...
        pResult = pStmt->executeQuery("SELECT `PayPlayMinute` FROM `PCRoomPayList` WHERE `PCRoomID`=%d AND `Year`=%d AND `Month`=%d", m_PCRoomID, vsDate.year(), vsDate.month());
        
        if (pResult->next()) {
            pStmt->executeQuery("UPDATE `PCRoomPayList` SET `PayPlayMinute`=`PayPlayMinute`+%d WHERE `PCRoomID`=%d AND `Year`=%d AND `Month`=%d", mm, m_PCRoomID, vsDate.year(), vsDate.month());
        } else {
            pStmt->executeQuery("INSERT INTO `PCRoomPayList` (`PCRoomID`, `Year`, `Month`, `PayPlayMinute`) VALUES (%d, %d, %d, %d)", m_PCRoomID, vsDate.year(), vsDate.month(), mm);
        }
...
bool PaySystem::isPayPlayingPeriodPersonal(const string& PlayerID) throw(Error) {
    __BEGIN_TRY
    ...
    BEGIN_DB {
        ...
        Result* pResult = pStmt->executeQuery("SELECT `PayType` = 0 or `PayPlayDate` > now() FROM `Player` WHERE `PlayerID`='%s'", PlayerID.c_str());
...
//---------------------------------------------------------------------------
bool PaySystem::isPlayInPayPCRoom(const string& ip, const string& playerID) throw(Error) {
    __BEGIN_TRY
    ...
    BEGIN_DB {
        ...
        try {
            pResult = pStmt->executeQuery("SELECT r.ID, r.PayType, r.PayStartDate, r.PayPlayDate, r.PayPlayHours FROM PCRoomInfo r, PCRoomIPInfo p WHERE p.IP='%s' AND p.ID=r.ID", ip.c_str());
        } catch (Throwable& t) {
            filelog("paySystem.txt", "%s", t.toString().c_str());
            throw;
        }
        ...
        if (pResult!=NULL && pResult->next()) {
            ...
            bool bAvailable = (payType == PAY_TYPE_FREE) || ((currentDateTime >= payStartAvailableDateTime) && (currentDateTime <= payPlayAvailableDateTime)) || (payPlayAvailableHours > 0);
