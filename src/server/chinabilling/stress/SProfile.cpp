...
void SProfile::print() {
    ...
    if (m_Count > 0 && m_IntervalCount > 0) {
        ...
        avg.tv_usec = (int)(((float)m_IntervalTotal.tv_sec / (float)m_IntervalCount) * 1000000 +
                            m_IntervalTotal.tv_usec / m_IntervalCount);
    ...
    sprintf(msg, "avg : %d.%06d count : %d time : %d.%06d\n", (int)avg.tv_sec, (int)avg.tv_usec, m_IntervalCount,
            (int)m_IntervalTotal.tv_sec, (int)m_IntervalTotal.tv_usec);
    printf(msg);
    filelog("perform.txt", msg);
    sprintf(msg, "total avg : %d.%06d max : %d.%06d min : %d.%06d total count : %d total time : %d.%06d\n",
            (int)totalavg.tv_sec, (int)totalavg.tv_usec, (int)m_Max.tv_sec, (int)m_Max.tv_usec, (int)m_Min.tv_sec,
            (int)m_Min.tv_usec, m_Count, (int)m_Total.tv_sec, (int)m_Total.tv_usec);
    printf(msg);
    filelog("perform.txt", msg);
